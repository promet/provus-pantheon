...
namespace Drupal\provus\Installer;

use Drupal\Core\DependencyInjection\ContainerInjectionInterface;
use Drupal\Core\Extension\Extension;
use Drupal\Core\Extension\ModuleExtensionList;
use Drupal\Core\Serialization\Yaml;
use Drupal\provus\Installer\ExtensionException;
use Symfony\Component\DependencyInjection\ContainerInterface;

/**
 * Extension Manager.
 *
 * Used to find extensions in provus that can be enabled during install.
 */
class ProvusExtensionManager implements ContainerInjectionInterface {
  ...
  /**
   * Collects the extensions for provus.
   *
   * @return array
  ...
  /**
   * Loads extensions from a `profile.provus_extension_list.yml` file.
   *
   * @return array
   *   An array containing the extensions with module name as the key.
   */
   ...
   *
   * @throws \Drupal\provus\Installer\ExtensionException
  ...
  protected function validateExtensionData(string $module_name, array $info) : bool {
    if (!isset($info['name'])) {
      throw new ExtensionException("The `name` field is missing in the `{$module_name}.provus_extension.yml` file.");
    }
    if (!isset($info['description'])) {
      throw new ExtensionException("The `description` field is missing in the `{$module_name}.provus_extension.yml` file.");
    }
    if (!is_bool($info['default'])) {
      throw new ExtensionException("The field `default` must be of type `bool` in the `{$module_name}.provus_extension.yml` file.");
    }
    if (!is_int($info['weight'])) {
      throw new ExtensionException("The field `weight` must be of type `int` in the `{$module_name}.provus_extension.yml` file.");
    }
